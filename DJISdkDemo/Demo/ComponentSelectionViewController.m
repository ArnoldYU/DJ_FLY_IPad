...
// 使用DJIBaseProduct的components属性初始化表视图。
...
//每种类型的组件都有相应的密钥名称。 我们创建一个字典来映射密钥名称和对应的视图。
...
//回调用于组件的连接更改。
// 1.当连接的组件断开连接（newComponent为零）时，我们删除表中相应的项。
// 2.当一个新组件连接（newComponent不为零）时，我们将一个项目添加到表中。
